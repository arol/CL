...
codechain CodeGenInstruction(AST *a,string info="")
{
	...
	if (a->kind=="list") {
		...
		string labelWhile=itostring(newLabelWhile());
		
		c="etiq while_"+ labelWhile ||
			GenRight(a->down, 0) ||
			"fjmp t0 endwhile_"+labelWhile ||
			CodeGenInstruction(a->down->right) ||
			"ujmp while_"+labelWhile ||
			"etiq endwhile_"+labelWhile;
			
	}
	else if(a->kind=="if"){
		string labelIf=itostring(newLabelIf());
		
		c=GenRight(a->down, 0);
		
		if(!a->down->right->right){
			c=c ||
				"fjmp t0 endif_"+labelIf ||
				CodeGenInstruction(a->down->right) ||
//					CodeGenInstruction(a->down->right->right) ||
				"etiq endif_"+labelIf;
		}else{
			c=c ||
				"fjmp t0 else_"+labelIf ||
				CodeGenInstruction(a->down->right) ||
				"ujmp endif_"+labelIf ||
				"etiq else_"+labelIf ||
				CodeGenInstruction(a->down->right->right) ||
		...
		codechain pushparams;
		codechain removeparams;
		
		CodeGenRealParams(a->down->right->down, symboltable[a->down->text].tp->down, pushparams, removeparams, 0);
